...
//        int code = R+1;  // R is codeword for EOF
        int code = R;   //TODO thats like hacking a princeton implementation - princeton decoding wont work now, but encoding tests pass

        while (input.length() > 0) {
            String s = st.longestPrefixOf(input);  // Find max prefix match s.
//            BinaryStdOut.write(st.get(s), W);      // Print s's encoding.
            PrincetonLZWOutput.getEncodingOutput().add(st.get(s));
